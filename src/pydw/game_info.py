...
class GameInfo:
    ...
    def __init__(
        ...
        self.tiles, self.tile_symbols, self.tile_probabilities = GameInfo.parse_tiles(
    ...
    def init_audio_player_music(xml_root: ET.Element, data_path: str) -> None:
        ...
        for element in xml_root.findall("./MusicMappings"):
            ...
            for track_element in element.findall("./Track"):
                ...
                name_to_music_track_mapping[name] = MusicTrack(
                    name,
                    source,
                    source2,
                    link,
    ...
    def init_audio_player_sounds(xml_root: ET.Element, data_path: str) -> None:
        ...
        for element in xml_root.findall("./SoundMappings"):
            ...
            for track_element in element.findall("./Track"):
                ...
                name_to_sound_track_mapping[name] = SoundTrack(
                    name,
                    source,
                    link,
                    track_element.attrib["credits"],
    ...
    def parse_map_locations(
        ...
        for element in xml_root.findall("./Maps//Map"):
            ...
            for location_element in element.findall("MapLocation"):
                ...
                map_locations[location_name] = NamedLocation(
                    location_name,
                    Point(
                        int(location_element.attrib["x"]),
    ...
    def parse_character_types(
        ...
        for element in xml_root.findall("./CharacterTypes//CharacterType"):
            ...
            if (
                ...
                dest_image_rect = pygame.Rect(
                    (tile_size_pixels - phase_image_scaled_size.w) // 2,
    ...
    def parse_monster(
        ...
        if 0 == black_border_pixels:
            ...
            scaled_monster_image = pygame.transform.scale(
                unscaled_monster_image,
                (
                    unscaled_monster_image.get_width() * monster_scale_factor,
                    unscaled_monster_image.get_height() * monster_scale_factor,
            ...
            monster_image = pygame.surface.Surface(
                (
                    scaled_monster_image.get_width() + 2 * black_border_pixels,
        ...
        for element in xml_root.findall("./Maps//Map"):
            ...
            def parse_outgoing_transition(
                ...
                if (
                    ...
                    and "rightX" in trans_element.attrib
                    and "topY" in trans_element.attrib
                    and "bottomY" in trans_element.attrib
                ):
    ...
    def parse_dialog(
        ...
        for element in dialog_root_element:
            ...
            if element.tag == "DialogGoTo":
                ...
                dialog.append(
                    DialogCheck(
                        DialogCheckEnum[element.attrib["type"]],
