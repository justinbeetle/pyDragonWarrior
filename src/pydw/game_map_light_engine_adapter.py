...
class GameMapLightEngineAdapter():
    ...
    def apply_dynamic_lighting(self, surface: pygame.surface.Surface) -> None:
        ...
        def add_point_light(
            ...
            # Calculate light posisition in screen pixels (self.map_layer.translate_rect/translate_point translate
            # from map pixels to screen pixels).
            light_map_pos_px = (self.image_pad_tiles + light_position_tiles) * self.tile_size_pixels
            light_screen_pos_px = Point(self.map_layer.translate_point(light_map_pos_px))
            ...
            if use_simple_logic:
                ...
                def is_visible(
                    ...
                    this_tile_rect = tile_rect.move(tile * self.tile_size_pixels)
                ...
                max_left_x = light_x
                min_right_x = max_left_x + 1
                max_upper_y = light_y
                min_lower_y = max_upper_y + 1
                ...
                def add_shadow(tile_pos: Point, rect: pygame.Rect) -> None:
                    shadow_rects.append(self.map_layer.translate_rect(rect.move(tile_pos * self.tile_size_pixels)))
                ...
                for x in range(max_left_x, min_x - 1, -1):
                    for y in range(min_y, max_upper_y + 1):
                        tile = Point(x, y)
                        if is_wall(tile):
                            is_visible(tile, check_ul=False, check_ur=True, check_lr=False, check_ll=True)
                for x in range(max_left_x, min_x - 1, -1):
                    for y in range(min_y, max_upper_y + 1):
                        tile = Point(x, y)
                        if is_wall(tile):
                            ...
                            if not is_wall(tile.get_left()):
                                add_shadow_left(tile)
                print(f"wall_tile_rects={wall_tile_rects}")

                # Handle lower left diagnal
                wall_tile_rects = []
                for x in range(min_x, max_left_x + 1):
            ...
            light = Light(light_radius_px, pygame.Color(255, 255, 255), 0.6)
