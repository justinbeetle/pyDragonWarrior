    ...
    def stage_track(self) -> bool:
        ...
        if not self.is_file1_present():
            if not self.is_file2_present():
                print(
                    f"ERROR: Failed to stage {self.file_path1} and {self.file_path2}",
                    flush=True,
                )
            else:
...
class AudioPlayer:
    class __AudioPlayer:
        ...
        def stage_music_track(self, track_name: str) -> Optional[MusicTrack]:
            ...
            if 0 == len(tracks):
                print(
                    f"ERROR: Failed to stage all possible music tracks with name {track_name}",
                    flush=True,
                )
                return None
            return tracks[0]
        ...
        def stage_sound_track(self, track_name: str) -> Optional[SoundTrack]:
            ...
            if 0 == len(tracks):
                print(
                    f"ERROR: Failed to stage all possible sound tracks with name {track_name}",
                    flush=True,
                )
                return None
            return tracks[0]
